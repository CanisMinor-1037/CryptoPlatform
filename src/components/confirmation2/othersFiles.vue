...
export default {
  ...
  methods: {
    downloadFile(row) {
      this.$http
        .post("/api/confirmation/downloadDbAsset", row)
        .then((res)=>{
            if(res.data.code!=200){
              this.$notify.warning("链接解析失败");
              return;
            }
            //alert("地址："+res.data.data.url+"\n用户名："+res.data.data.username+"\n密码："+res.data.data.password);
            let info={
              url:res.data.data.url,
              username:res.data.data.username,
              password:res.data.data.password,
              assetFabricId:row.fabricId
            }
            this.$store.commit('setDatabaseInfo',info);
    ...
    getData() {
      this.$http.post("/api/confirmation/databaseAvailable").then((res)=>{
